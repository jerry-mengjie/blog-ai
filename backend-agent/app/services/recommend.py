"""推荐缓存服务: 在 LangGraph 推荐图外层加经典 L1 + L2 多级缓存。

缓存策略:
1. 匿名请求按 size 聚合为高命中共享 key
2. 登录请求按 user_id + size 分 key, 使用短 TTL 平衡实时性与性能
3. 文章增删改后由 backend-blog 调本服务的失效接口统一清缓存;
   浏览/收藏/标签变更依赖短 TTL 自然收敛

性能要点:
1. 缓存在 API 边界层, 命中时直接跳过整张图, 也就跳过了对 blog / rag 的全部网络调用
2. L1 同进程零网络, L2 Redis 跨副本共享
...
# 导入 LangGraph 推荐入口
from app.graphs.recommend import recommend_articles
...
# 对外入口: 先查缓存, miss 再执行 LangGraph 推荐图
async def list_recommended_articles(user_id: int | None, size: int) -> list[dict]:
    ...
    # 回源闭包: 真 miss 时才执行图(进而调用 blog / rag)
    async def _factory() -> list[dict]:
        # 执行推荐图, 返回已装配好的卡片列表
        return await recommend_articles(user_id, normalized_size)

    # L1 → L2 → LangGraph 推荐图
    return await _rec_cache.get_or_set(key, _factory)
